...
<body style="margin: 0;padding: 0;height: 100%;overflow: hidden;">
   <!-- Loading 動畫 -->
	...
	(async () => {		
		...
		// 載入資料並展示進度
		//--
		// 第1個階段載入工作: 用等待500ms代替
		await new Promise(r => setTimeout(r, 500));// 等待500ms
		// 設定為完成33%，這是依照數量做的進度不是用運行時間作為進度
		updateProgressBar(33);
		//--
		// 第2個階段載入工作: 用等待1000ms代替
		await new Promise(r => setTimeout(r, 1000));// 等待1000ms
		//設定為完成66%，這是依照數量做的進度不是用運行時間作為進度
		updateProgressBar(66);
		//--
		// 第3個階段載入工作: 用等待500ms代替
		await new Promise(r => setTimeout(r, 500));// 等待500ms
		//設定為完成100%，這是依照數量做的進度不是用運行時間作為進度
